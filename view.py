...
def criar_pedido():
    ...
    try:
        ...
        cur.execute("SELECT GEN_ID(GEN_PEDIDO_ID, 1) FROM RDB$DATABASE")
        id_pedido = cur.fetchone()[0]

        cur.execute("""
            INSERT INTO PEDIDO (ID_PEDIDO, ID_MESA, DATA_HORA, VALOR_TOTAL, OBSERVACOES)
            VALUES (?, ?, CURRENT_TIMESTAMP, ?, ?)
        """, (id_pedido, id_mesa, valor_total, observacoes))

        for item in itens_completos:
            cur.execute("SELECT GEN_ID(GEN_ITEM_PEDIDO_ID, 1) FROM RDB$DATABASE")
            id_item = cur.fetchone()[0]
            cur.execute("""
                INSERT INTO ITEM_PEDIDO (ID_ITEM, ID_PEDIDO, ID_PRODUTO, QUANTIDADE, PRECO_UNITARIO, SUBTOTAL)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (id_item, id_pedido, item['id_produto'], item['quantidade'], item['preco_unitario'], item['subtotal']))
